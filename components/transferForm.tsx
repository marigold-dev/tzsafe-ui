...
function get(s: string | FormikErrors<{ to: string; amount: string; }>): boolean {
    if (typeof s == "string") {
        return false
    } else {
        if (s.to || s.amount) {
...
function TransferForm(props: React.PropsWithoutRef<{ address: string, closeModal: () => void }>) {
    ...
    async function transfer(txs: { to: string; amount: number; }[]) {
        ...
        let params = cc.methods.create_proposal(txs.map(x => ({ transfer: { target: x.to, amount: x.amount, parameter: {} } }))).toTransferParams()
    ...
    return (
        <Formik
            ...
            validate={(values) => {
                console.log(values)
                const errors: { transfers: { to: string, amount: string }[] } = { transfers: [] };
                let dedup = new Set();
                let result = values.transfers.map(x => {
                    ...
                    if (dedup.has(x.to)) {
                        ...
                        dedup.add(x.to)
                        return ({ to: validateAddress(x.to) !== 3 ? `invalid address ${x.to}` : "", amount: "" })
                    }
                });
                        ...
                        <FieldArray name="transfers">
                            {({ remove, push }) => (
                                <div className="min-w-full">
                                    {values.transfers.length > 0 && values.transfers.map((transfer, index) => {
                                        return (

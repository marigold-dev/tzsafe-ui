const v0_3_4 = `{ parameter
    (or (or (or (list %create_proposal
                   (or (or (or (or (pair %add_or_update_metadata (string %key) (bytes %value))
                           ...
                           (or (or (pair %execute_lambda
                                   ...
                                   (bytes %proof_of_event))
                               (or (string %remove_metadata) (set %remove_owners address))))
                       (pair %transfer (address %target) (mutez %amount))))
                (unit %default))
            (or (bytes %proof_of_event_challenge)
  ...
  code { PUSH string "Threshold must be greater than 1" ;
           ...
           (pair nat
                 ...
                 (big_map
                    ...
                    (pair (or (or unit unit) (or unit unit))
                          ...
                          (list (or (or (or (or (pair string bytes) (set address)) (or int nat))
                                        (or (or (pair (lambda unit (list operation)) (option bytes)) bytes) (or string (set address))))
                                    (pair address mutez)))))
         ...
         LAMBDA
           (pair (list (or (or (or (or (pair string bytes) (set address)) (or int nat))
                               (or (or (pair (lambda unit (list operation)) (option bytes)) bytes) (or string (set address))))
                           (pair address mutez)))
                 (list (or (or (or (or (pair string bytes) (set address)) (or int nat))
                               (or (or (pair (lambda unit (list operation)) (option bytes)) bytes) (or string (set address))))
           ...
           { IF_LEFT
               ...
               { IF_LEFT
                   { DIG 3 ;
                     ...
                     DROP 7 ;
                     SWAP ;
                     NIL (or (or (or (or (pair string bytes) (set address)) (or int nat))
                                 (or (or (pair (lambda unit (list operation)) (option bytes)) bytes) (or string (set address))))
                             (pair address mutez)) ;
                     DIG 2 ;
                     RIGHT (pair (lambda unit (list operation)) (option bytes)) ;
                     LEFT (or string (set address)) ;
                     RIGHT (or (or (pair string bytes) (set address)) (or int nat)) ;
                     LEFT (pair address mutez) ;
                     CONS ;
                     PAIR ;
                     EXEC }
                   { DIG 2 ;
                                      ...
                                      { IF_LEFT
                                          ...
                                          { IF_LEFT
                                              { IF_LEFT
                                                  ...
                                                  { EMIT %proof_of_event bytes ; DIG 2 ; NIL operation ; DIG 2 ; CONS } }
                                              { IF_LEFT
                                                  { DIG 2 ; NONE bytes ; DIG 2 ; PAIR ; PAIR ; DUP 5 ; SWAP ; EXEC }
                                                  { DUP 3 ;
                                                    DIG 3 ;
                                                    GET 7 ;
                                                    DIG 2 ;
